...
public class PathfindingAgent : MonoBehaviour
{
    ...
    public virtual void Arrive(Vector3 position, bool useInfluence, float acceptanceRange = 0, Action completionAction = null)
    {
        if (!activated) return;
        Stop();
    ...
    // Chase the target.
    public virtual void Chase(Transform target, bool useInfluence, float acceptanceRange = 0, Action completionAction = null)
    {
        if (!activated) return;
        Stop();
        arrivalAcceptanceRange = acceptanceRange;
